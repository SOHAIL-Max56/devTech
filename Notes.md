...
**Question: How are variable and function private in different module?**

Answer: IIFE and require statement which wraps function inside the IIFE

**Question: How do you get access to module.exports and require?**

Answer: NodeJS wraps the module code inside the function which provides access to `module`, `exports`, `require`, `__filename`, `__dirname` etc.
...
1. **Creating a Simple Web Server using NodeJS**
...
1. **Requirements for DevTech Blog Application**
...
2. **Updating Documents in MongoDB using Mongoose**
   ...
   - There are several methods to update documents in MongoDB using Mongoose. Some of the commonly used [Read more at Mongoose Doc](https://mongoosejs.com/docs/api/model.html)
...
4. **API Validation for Update Operations**
...
1. **Flow of Signup and Login API**
...
## Episode (Building DevTech Blog Application - Part 4)

1. **Compound Indexes in MongoDB**

- A compound index is an index on multiple fields that improves query performance when filtering or sorting by several fields simultaneously.
...
- `{ senderId: 1, receiverId: 1 }` creates an index on both `senderId` and `receiverId` fields. The `1` indicates ascending order (use `-1` for descending).
- `{ unique: true }` enforces uniqueness on the combination of both fields. This prevents duplicate connection requests between the same sender and receiver.
- Example: If user A sends a request to user B, MongoDB won't allow another identical request from A to B.
